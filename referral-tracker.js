 ...
 *
 * The BACKEND_URL is auto-detected from the script src attribute.
 * If loaded directly (not via <script src>), set BACKEND_URL manually.
 * ─────────────────────────────────────────────────────────
 */
(function () {
  // Auto-detect backend URL from the script tag that loaded this file
  var BACKEND_URL = (function() {
    ...
    // No fallback — must set window.__REF_BACKEND_URL before loading tracker
    return (window.__REF_BACKEND_URL || '');
  })();
  ...
  setInterval(autoAttachClaim, 2000);
